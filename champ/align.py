...
def run(cluster_strategy, rotation_adjustment, h5_filenames, path_info, snr, min_hits, fia, end_tiles, alignment_channel, all_tile_data, metadata, make_pdfs, sequencing_chip, process_limit, floor_alignment):
    ...
    alignment_func = functools.partial(perform_alignment, cluster_strategy, rotation_adjustment, path_info, snr, min_hits, metadata['microns_per_pixel'],
                                       sequencing_chip, all_tile_data, make_pdfs, fia, floor_alignment)
...
def perform_alignment(cluster_strategy, rotation_adjustment, path_info, snr, min_hits, um_per_pixel, sequencing_chip, all_tile_data,
                      make_pdfs, prefia, floor_alignment, image_data):
    ...
    try:
        ...
        fia = process_alignment_image(cluster_strategy, rotation_adjustment, snr, sequencing_chip, base_name, um_per_pixel, image, possible_tile_keys, deepcopy(prefia), floor_alignment)
...
def get_end_tiles(cluster_strategies, rotation_adjustment, h5_filenames, alignment_channel, snr, metadata, sequencing_chip, fia, floor_alignment, full_search):
    right_end_tiles = {}
    left_end_tiles = {}
    ...
    if full_search:
        left_side = all_tiles
        right_side = all_tiles
    else:
        left_side = sequencing_chip.left_side_tiles
        right_side = sequencing_chip.right_side_tiles
    ...
    for cluster_strategy in cluster_strategies:
        with h5py.File(h5_filenames[0]) as first_file:
            ...
            base_column_checker = functools.partial(check_column_for_alignment, cluster_strategy, rotation_adjustment, alignment_channel, snr, sequencing_chip, metadata['microns_per_pixel'], fia, floor)
            left_end_tiles = dict(find_bounds(pool, h5_filenames, base_column_checker, grid.columns, left_side))
            right_end_tiles = dict(find_bounds(pool, h5_filenames, base_column_checker, reversed(grid.columns), right_side))
            pool.close()
            pool.join()
...
def check_column_for_alignment(cluster_strategy, rotation_adjustment, channel, snr, sequencing_chip, um_per_pixel, fia, floor,
    ...
    with h5py.File(h5_filename) as h5:
        ...
        for row in rows_to_check:
            ...
            fia = process_alignment_image(cluster_strategy, rotation_adjustment, snr, sequencing_chip, base_name, um_per_pixel, image, possible_tile_keys, deepcopy(fia), floor)
            if fia.hitting_tiles:
...
def process_alignment_image(cluster_strategy, rotation_adjustment, snr, sequencing_chip, base_name, um_per_pixel, image, possible_tile_keys, fia, floor):
