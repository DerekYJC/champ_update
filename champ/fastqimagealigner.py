...
class FastqImageAligner(object):
    ...
    def find_hitting_tiles(self, floor_alignment, possible_tile_keys, snr_thresh=1.2):
        ...
        log.debug("Image shape of (%d , %d) having the FFT of shape (%d , %d)" % (self.image_data.image.shape[0], self.image_data.image.shape[1],
    ...
    def find_hits(self, consider_tiles='all'):
        ...
        if int(16.0 / self.image_data.um_per_pixel) == 60:
            ...
            good_hit_threshold = np.percentile(self.hit_dists(exclusive_hits), 97)
        # second_neighbor_thresh = 2 * good_hit_threshold
        second_neighbor_thresh = 1.2 * good_hit_threshold
        log.debug("Good hit threshold is %f , Second neighbor threshold is %f" % (good_hit_threshold, second_neighbor_thresh))
